...
if (count.count === 0) {
    const insert = db.prepare(`
        INSERT INTO recipes (title, ingredients, instructions, prep_time, cook_time, servings, difficulty, favorite, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) 
    `);

    insert.run('Rice', 'water, rice, oil, salt', 'add ingredients, cook', 4, 20, 4, 'easy', 1, new Date().toISOString());
    insert.run('Beans', 'water, beans, oil, salt, seasoning', 'add ingredients, cook', 7, 30, 6, 'medium', 0, new Date().toISOString());
    insert.run('Pork Chops', 'Pork Chop, seasoning, oil', 'add ingredients, cook', 2, 10, 2, 'hard', 0, new Date().toISOString());
...
app.get('/api/recipes', (req, res) => {
    ...
    if (filter === 'easy') {
        recipes = db.prepare(query + " WHERE difficulty = 'easy'").all();
    } else if (filter === 'medium') {
        recipes = db.prepare(query + " WHERE difficulty = 'medium'").all();
    } else if (filter === 'hard') {
        recipes = db.prepare(query + " WHERE difficulty = 'hard'").all();
    } else if (filter === 'favorite') {
        recipes = db.prepare(query + " WHERE favorite = 1").all();
    } else if (filter === 'notFavorite') {
        recipes = db.prepare(query + " WHERE favorite = 0").all();
    } else {
        recipes = db.prepare(query).all();
    }
...
app.post('/api/recipes', (req, res) => {
    const { title, ingredients, instructions, prep_time, cook_time, servings, difficulty, favorite } = req.body;

    if (!title || !ingredients || !instructions || !prep_time || !cook_time || !servings || !difficulty || !favorite) {
        return res.status(400).json({
            error: "Missing required fields",
            required: ["title", "ingredients", "instructions", "prep_time", "cook_time", "servings", "difficulty", "favorite"]
        });
    }
    ...
    const result = insert.run(title, ingredients, instructions, prep_time, cook_time, servings, difficulty, favorite ? 1 : 0, new Date().toISOString());
...
app.put('/api/recipes/:id', (req, res) => {
    ...
    const update = db.prepare(`
        UPDATE recipes
        SET title = ?, ingredients = ?, instructions = ?, prep_time = ?, cook_time = ?, servings = ?, difficulty = ?, favorite = ?
        WHERE ID = ?    
    `);
...
app.patch('/api/recipes/:id/toggle', (req, res) => {
    ...
    db.prepare('UPDATE recipes SET favorite = ? WHERE id = ?').run(newFavoriteStatus, id);
...
app.get('/api/stats', (req, res) => {
    ...
    const favorite = db.prepare('SELECT COUNT(*) as count FROM recipes WHERE favorite = 1').get().count;
